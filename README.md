...
- 自动测试不调用付费模型；实际模型任务 #12 的 10 个测试点已用独立算法重算，全部答案一致，
  并包含 `N=200000` 的复杂度区分数据。
...
验证记录：完整回归 299 项通过（其中包含 6 项题库验证），另通过真实 Chromium 验证登录、刷新、
题目与 AI 详情、浏览器返回、页面返回、访问审计和登出流程。
