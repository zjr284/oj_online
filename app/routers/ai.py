...
- PUT  /api/ai/model-config                 配置模型（仅管理员；api_key 加密存储，永不返回）
- GET  /api/ai/model-config                 读取配置的公开字段（等价扩展：api_key 仍不返回）
- POST /api/ai/problem-tasks/               创建命题任务（仅管理员；无配置 400、题目不存在 404）
- GET  /api/ai/problem-tasks/{task_id}          任务状态（仅管理员）
- GET  /api/ai/problem-tasks/{task_id}/events   SSE 进度事件（仅管理员）
...
async def set_model_config(body: ModelConfigIn, user: User = Depends(require_admin)):
...
async def create_problem_task(body: AiTaskIn, user: User = Depends(require_admin)):
...
async def list_problem_tasks(user: User = Depends(require_admin)):
    """仅管理员可查询命题任务列表；列表项不含 result。"""
    return ok(await ai_service.list_tasks(user))


@router.get("/problem-tasks/{task_id}")
async def get_problem_task(task_id: int, user: User = Depends(require_admin)):
    return ok(await ai_service.get_task(user, task_id))


@router.get("/problem-tasks/{task_id}/events")
async def get_problem_task_events(task_id: int, user: User = Depends(require_admin)):
...
async def cancel_problem_task(task_id: int, user: User = Depends(require_admin)):
