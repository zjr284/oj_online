...
评测明细按管理员、本人及题目公开策略裁剪。允许与拒绝的日志查询写入访问审计；审计界面按用户名展示和筛选，每条记录末尾同时提供详情与删除操作，删除需要二次确认。删除题目不会抹掉历史评测；没有题目公开策略时默认关闭他人访问。
...
完整回归 295 项通过。n=200000、q=50000 的构造数据中，Python 和 C++14 二分解法 AC，Python 线性扫描 TLE；三个随仓库发布的题目样例及测试点均经独立计算校验。实际模型任务 #12“冒泡排序的交换次数”的 10 个测试点也经树状数组独立重算全部一致，其中包含 `N=200000` 的递增、递减、重复和周期数据，可区分 `O(N log N)` 与 `O(N²)` 解法。真实 Chromium 还验证了登录、整页刷新、题目与 AI 详情、浏览器返回、页面内返回、审计按钮及登出，全程保持单标签和正确登录态。详见 EXPERIMENT2_AUDIT.md。
