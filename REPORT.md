...
Streamlit 页面覆盖注册、登录/退出、用户名修改、个人信息、用户管理、动态语言注册、题目列表/详情/新增/编辑/删除、代码提交、评测记录和日志。题目、用户、评测记录、访问审计和 AI 任务列表的分页统一显示总页数并支持指定页跳转。页面统一调用后端 API，后端负责所有权限判断。浏览器 Cookie 支持刷新后恢复身份，`st.navigation` 原生路由支持浏览器前进和返回。
...
路由在解析 JSON 前检查身份和管理员权限，从而满足 401/403 优先于参数错误的约定。参数错误返回 400，所有响应包含与 HTTP 状态一致的 code。所有登录用户可创建题目、注册语言及使用 AI 命题；编辑、删除题目与修改日志公开策略仅管理员可操作。

评测明细按管理员、本人及题目公开策略裁剪。评测列表可组合用户、题目和状态筛选，也允许管理员只按状态筛选。允许与拒绝的日志查询写入访问审计；审计界面按用户名展示和筛选，每条记录末尾提供删除操作并要求二次确认。删除题目不会抹掉历史评测；没有题目公开策略时默认关闭他人访问。

### 2.4 AI 命题工作流

用户配置服务 URL、模型、密钥及单价，密钥用 Fernet 加密保存，保存结果不回显密钥。创建任务时固定配置，避免运行期间变更配置影响该次调用。

后台调用模型、校验返回 JSON，再把结果保存在任务中。前端提供题面、样例、实际测试点和 JSON 编辑，用户审阅后通过原有题目 API 保存；改编保持原题 id，另存为新题目时若题号已存在会原子分配新题号。已完成结果支持继续对话修改，每轮以上一版完整 JSON 和本轮意见生成不可变的新版本，保留题目 id，并提供完整修改链回看。提示词要求覆盖知识点、边界和能区分算法复杂度的测试。

进度存入数据库，前端每 1.5 秒进行片段轮询，后端另提供 SSE；不同订阅者各自持有事件队列。中断会取消并等待后台任务结束。模型调用使用默认 600 秒、可配置的单一总超时，且默认忽略宿主机遗留代理。DeepSeek 官方接口支持极速（Flash/关闭思考）、均衡（Flash/low）和高质量（Pro/high）三档任务模式；高质量模式在初稿之后额外执行完整终审，核对标准输出、输入数量、边界与复杂度数据，最终只保存修正版，两次调用仍共用一个总超时。失败任务使用当前密钥和接口配置并沿用原模式创建新任务，原错误和用量保留。

费用优先采用提供方直接返回值；DeepSeek 官方接口按实际 Flash/Pro 模型、请求峰谷时段以及缓存命中/未命中 Token 自动计算，其他兼容服务使用成对配置的输入/输出单价。费用最多显示 8 位小数，零费用明确显示为零；界面只保留 Token 与最终费用。重试累加每次用量；校验失败保留已发生的用量。缺失 Token 按包含系统提示词的字符数估算并标注；缺少安全计价条件时保持未知，不伪造金额。
...
完整回归 303 项通过。n=200000、q=50000 的构造数据中，Python 和 C++14 二分解法 AC，Python 线性扫描 TLE；三个随仓库发布的题目样例及测试点均经独立计算校验。实际模型任务 #12“冒泡排序的交换次数”的 10 个测试点也经树状数组独立重算全部一致，其中包含 `N=200000` 的递增、递减、重复和周期数据，可区分 `O(N log N)` 与 `O(N²)` 解法。真实 Chromium 还验证了登录、整页刷新、题目与 AI 详情、浏览器返回、页面内返回、审计按钮及登出，全程保持单标签和正确登录态。详见 EXPERIMENT2_AUDIT.md。
